...
class DreameStatus:
    ...
    # )
    battery: int = field(
        metadata={"siid": 3, "piid": 1, "access": ["read", "notify"]}, default=None
    )
    ...
    # piid: 1 (Status): (int8, unit: None) (acc: ['read', 'notify'], value-list: [{'value': 1, 'description': 'Sweeping'}, {'value': 2, 'description': 'Idle'}, {'value': 3, 'description': 'Paused'}, {'value': 4, 'description': 'Error'}, {'value': 5, 'description': 'Go Charging'}, {'value': 6, 'description': 'Charging'}, {'value': 7, 'description': 'Mopping'}], value-range: None)
    status: int = field(
        metadata={
            "siid": 2,
            "piid": 1,
    ...
    filter_life_level: int = field(
        metadata={"siid": 11, "piid": 1, "access": ["read", "notify"]}, default=None
    )
    # piid: 2 (Filter Left Time): (uint16, unit: hour) (acc: ['read', 'notify'], value-list: [], value-range: [0, 300, 1])
    filter_left_time: int = field(
        metadata={"siid": 11, "piid": 2, "access": ["read", "notify"]}, default=None
    )

    # siid 10: (Side Cleaning Brush): 2 props, 1 actions
    # piid: 1 (Brush Left Time): (uint16, unit: hour) (acc: ['read', 'notify'], value-list: [], value-range: [0, 300, 1])
    brush_left_time2: int = field(
        metadata={"siid": 10, "piid": 1, "access": ["read", "notify"]}, default=None
    )
    # piid: 2 (Brush Life Level): (uint8, unit: percentage) (acc: ['read', 'notify'], value-list: [], value-range: [0, 100, 1])
    brush_life_level2: int = field(
        metadata={"siid": 10, "piid": 2, "access": ["read", "notify"]}, default=None
    )

    # siid 4: (clean): 16 props, 2 actions
    # piid: 1 (工作模式): (int32, unit: none) (acc: ['read', 'notify'], value-list: [], value-range: [0, 17, 1])
    operating_mode: int = field(
        metadata={"siid": 4, "piid": 1, "access": ["read", "notify"]}, default=None
    )
    # piid: 4 (area): (string, unit: None) (acc: ['read', 'write'], value-list: [], value-range: None)
    area: str = field(
        metadata={"siid": 4, "piid": 3, "access": ["read", "write"]}, default=None
    )
    # piid: 5 (timer): (string, unit: None) (acc: ['read', 'write'], value-list: [], value-range: None)
    timer: str = field(
        metadata={"siid": 4, "piid": 2, "access": ["read", "write"]}, default=None
    )
    # piid: 4 (清扫模式): (int32, unit: none) (acc: ['read', 'write', 'notify'], value-list: [{'value': 0, 'description': '安静'}, {'value': 1, 'description': '标准'}, {'value': 2, 'description': '中档'}, {'value': 3, 'description': '强力'}], value-range: None)
    fan_speed: int = field(
        metadata={
            "siid": 4,
            "piid": 4,
    ...
    # piid: 13 (): (uint32, unit: minutes) (acc: ['read', 'notify'], value-list: [], value-range: [0,   , 1])
    last_clean: int = field(
        metadata={"siid": 12, "piid": 1, "access": ["read", "notify"]}, default=None
    )
    # piid: 14 (): (uint32, unit: None) (acc: ['read', 'notify'], value-list: [], value-range: [0, 4294967295, 1])
    total_clean_count: int = field(
        metadata={"siid": 12, "piid": 2, "access": ["read", "notify"]}, default=None
    )
    # piid: 15 (): (uint32, unit: None) (acc: ['read', 'notify'], value-list: [], value-range: [0, 4294967295, 1])
    total_area: int = field(
        metadata={"siid": 12, "piid": 4, "access": ["read", "notify"]}, default=None
    )
    ...
    map_view: str = field(
        metadata={"siid": 6, "piid": 1, "access": ["read", "notify"]}, default=None
    )
...
class DreameVacuum(MiotDevice):
    ...
    def start(self) -> None:
        ...
        # urn:dreame-spec:service:vacuum-extend
